...
public class ForgetPassPage extends JFrame{
    ...
    public ForgetPassPage(String username, String email, String verifCode) {
                ...
                if (newPass.length() >= 8) {
                    if (newPass.matches("[a-zA-Z0-9]+")) {
                        if (newPass.equals(conNewPass)) {
                            for (User user : users) {
                                if (user.getUsername().equals(username) && user.getEmail().equals(this.email)) {
                                    user.resetPass(this.username, this.email, newPass);
                                    JOptionPane.showMessageDialog(ForgetPassPage.this, "Your password for the user '" + this.username +
                                            "' has been changed!");
