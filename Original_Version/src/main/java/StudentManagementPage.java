...
public class StudentManagementPage extends JFrame {
    ...
    private static final String student_file_path = ResourceManager.getStudentDataPath();

    JButton undoBtn;
    private Student recentDeletedStudent;
    private int recentDeletedStudentIndex;
    private Timer undoTimer;
    private static final int timeout = 20000;

    public StudentManagementPage(User user) {
        this.user = user;
        this.students = Student.loadData(ResourceManager.getStudentDataPath());
        initFrame();
    }
    ...
    private void addStudentDialog(){
        ...
        addbtn.addActionListener(e -> {
            ...
            addStudent(id, firstName, lastName, major, year, email);
            dialog.dispose();
            loadStudentList();
        });
    ...
    private void addStudent(String id, String firstName, String lastName, String major, String year, String email) {
        try {
            ...
            if (chkFileEmpty) {
                writer.write("StudentID\tFirstName\tLastName\tMajor\tYear\tEmail");
                writer.newLine();
            }

            String newStudent = id + "\t" + firstName + "\t" + lastName + "\t" + major + "\t" + year + "\t" + email + "\tnull\tnull";
            writer.write(newStudent);
            writer.newLine();
            writer.close();

            students = Student.loadData(student_file_path);
            loadStudentList();
            JOptionPane.showMessageDialog(this, "Successfully added new student");
        } catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    private void deleteStudentDialog(Student student){
        ...
        if (confirm == JOptionPane.YES_OPTION){
            try{
                // Store the deleted student for undo purpose
                recentDeletedStudent = student;
                recentDeletedStudentIndex = students.indexOf(student);

                students.remove(student);

                BufferedWriter writer = new BufferedWriter(new FileWriter(student_file_path));
                ...
                students = Student.loadData(student_file_path);
                filterStudents();
                undoOption(); // show undo after deleting student
    ...
    private void undoDelete() {
        ...
        try {
            ...
            // Restore the student
            if (recentDeletedStudentIndex >= 0 && recentDeletedStudentIndex <= students.size()) {
                students.add(recentDeletedStudentIndex, recentDeletedStudent);
            } else {
                students.add(recentDeletedStudent);
            }
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error restoring student: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void editStudentDialog(Student student){
        ...
        saveBtn.addActionListener(e -> {
            ...
            updateStudent(student, firstName, lastName, email, major, year);
            dialog.dispose();
            filterStudents();
        });
    ...
    private void updateStudent(Student student, String firstName, String lastName, String email, String major, String year) {
        try {
