...
public class UserManagementPage extends JFrame {
    ...
    private static final String user_file_path = ResourceManager.getUserDataPath();

    JButton undoBtn;
    private User recentDeletedUser;
    private int recentDeletedUserIndex;
    private Timer undoTimer;
    private static final int timeout = 20000;

    public UserManagementPage(User user){
        this.user = user;
        this.users = User.loadData(ResourceManager.getUserDataPath());
        initFrame();
    }
    ...
    private void filterUsers() {
        ...
        List<User> filtered = users.stream().filter(user -> user.getRole().equals("Admin") || user.getRole().equals("AcademicOfficer")).filter(user -> {
    ...
    private void addUserDialog(){
        ...
        addbtn.addActionListener(e -> {
            ...
            addUser(username, password, firstName, lastName, email, role);
            dialog.dispose();
            loadUserList();
        });
    ...
    private void addUser(String username, String password, String firstname, String lastName, String email, String role) {
        try {
            File file = new File(user_file_path);
            boolean chkFileEmpty = (file.length() == 0);
            ...
            String newUser = username + "\t" + password + "\t" + firstname + "\t" + lastName + "\t" + email + "\t" + role + "\tnull\tnull";
            writer.write(newUser);
            writer.newLine();
            writer.close();

            users = User.loadData(user_file_path);
            loadUserList();
            JOptionPane.showMessageDialog(this, "Successfully added new user");
        } catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    private void deleteUserDialog(User user){
        ...
        if (confirm == JOptionPane.YES_OPTION){
            try{
                // Store the deleted user for undo purpose
                recentDeletedUser = user;
                recentDeletedUserIndex = users.indexOf(user);
                ...
                for (User u: users) {
                    String lastLogin = u.getLastLogin() != null ? String.valueOf(u.getLastLogin().getTime()) : "null";
                    String lastLogout = u.getLastLogout() != null ? String.valueOf(u.getLastLogout().getTime()) : "null";
    ...
    private void undoDelete() {
        ...
        try {
            ...
            // Restore the user
            if (recentDeletedUserIndex >= 0 && recentDeletedUserIndex <= users.size()) {
                users.add(recentDeletedUserIndex, recentDeletedUser);
            } else {
                users.add(recentDeletedUser);
            }
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error to restore user: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void editUserDialog(User user){
        ...
        saveBtn.addActionListener(e -> {
            ...
            updateUser(user, password, firstName, lastName, email, role);
            dialog.dispose();
            filterUsers();
        });
    ...
    private void updateUser(User user, String password, String firstName, String lastName, String email, String role) {
        try {
            ...
            writer.close();
            users = User.loadData(User.getUserFilePath());
