...
class DocumentService{
    ...
    //create document
    public DocumentService(PdfFile pdfFile){
        this.pdfFile = pdfFile;
        this.studentList = Student.loadData(ResourceManager.getStudentDataPath());
    }

    //create document with studentPersonalInfo
    public DocumentService(PdfFile pdfFile, boolean studentPersonalInfo){
        this.pdfFile = pdfFile;
    ...
    public boolean generateManyPerformanceReports(String logoPath,String yearStudy, String program, File directory)throws Exception{  
        for (Student s : studentList) {
            ...
            if (s.getMajor().equalsIgnoreCase(program) && s.getYear().equalsIgnoreCase(yearStudy)) {
